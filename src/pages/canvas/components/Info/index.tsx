...
const ShapeInfo = memo((props: {selectShape: BaseShape}) => {
  // const [selectShape, setSelectShape] = useState<any>()
  const [mode, setMode] = useState<ShapeType>(ShapeType.SELECT)
  const conf: BaseConfig = props.selectShape?.conf
  ...
  return (
    <>
        ...
        mode === ShapeType.EDIT ?
          ...
          <div className="base-info">
            ...
            <div className="row grid2">
              ...
              <div className="col">
                <BaseButton active={conf?.flipHorizontal} onClick={() => flip(0)}>
                  <FlipIcon style={{fontSize: "16rem", 'transform': 'rotate(-90deg)'}}/>
                </BaseButton>
                <BaseButton active={conf?.flipVertical} onClick={() => flip(1)}>
      ...
      {
        props.selectShape.conf.type === ShapeType.TEXT &&
        <div className="base-info">
      ...
      <div className="base-info" style={{lineBreak: 'anywhere'}}>
        <div dangerouslySetInnerHTML={{__html: props.selectShape.getStatus()}}>
        </div>
      </div>
    </>
  )
